...
module.exports = function ({nodeVersion, output, opencvVersion, flags}) {
	...
	})).then(() => spawn('./host/package-host-cmd.bash', [nodeVersion, '/tmp/opencv4nodejs-lambda-layer/', opencvVersion])).then(() => rimraf('/tmp/opencv4nodejs-lambda-layer/nodejs').then(() => mkdirp('/tmp/opencv4nodejs-lambda-layer/nodejs'))).then(() => fs.rename('/tmp/opencv4nodejs-lambda-layer/node_modules/', '/tmp/opencv4nodejs-lambda-layer/nodejs/node_modules/')).then(() => zipDirectory('/tmp/opencv4nodejs-lambda-layer/', output)).then(() => rimraf('/tmp/opencv4nodejs-lambda-layer')).then(() => {
